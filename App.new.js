...
function DashboardContent() {
  ...
  useEffect(() => {
      const fetchDashboardData = async () => {
          ...
          try {
              ...
              if (isToday) {
                  endpoint = `/api/live?horizon=${selectedPeriod === 24 ? 4 : selectedPeriod}`; // Safeguard horizon mapping if UI uses 24 default
              } else {
  ...
  const calculations = useMemo(() => {
    ...
    const totalKwh = (chartData.aggregate_total_kwh || 0) * loadMultiplier;
    const currentCost = totalKwh * effectiveTariff;

    // Distribute actual live MongoDB usage using static historical domain averages 
    // until Phase 2 appliance-level aggregation is implemented in the data endpoints
    const fanKwh = totalKwh * 0.15;
    const acKwh = totalKwh * 0.55;
    const refKwh = totalKwh * 0.30;
    ...
    return {
      ...
      selectedPeriodText,
      applianceData: {
        'Electric Fan': { kwh: fanKwh, php: fanPhp, data: processedChartData.actuals.map(w => w !== null ? w * 0.15 : null), forecast: processedChartData.forecasts.map(w => w !== null ? w * 0.15 : null) },
        'Air Conditioner': { kwh: acKwh, php: acPhp, data: processedChartData.actuals.map(w => w !== null ? w * 0.55 : null), forecast: processedChartData.forecasts.map(w => w !== null ? w * 0.55 : null) },
        'Refrigerator': { kwh: refKwh, php: refPhp, data: processedChartData.actuals.map(w => w !== null ? w * 0.30 : null), forecast: processedChartData.forecasts.map(w => w !== null ? w * 0.30 : null) },
      },
    };
  }, [chartData.aggregate_total_kwh, processedChartData, tariff, budget, selectedPeriod, isScenarioMode, scenarioParams]);
