...
What it does (per appliance):
  ...
  - Daily rolling-origin evaluation with horizon=24 hours:
      * At each daily origin (00:00), optionally refit the model
      * Training data ends at the previous day's 23:00
      * Forecast the full next day: 00:00 to 23:00
      * Collect and score all 24 hourly forecasts for that day
  - Produces:
...
def rolling_origin_eval(
    ...
    # Daily next-day origins only: midnight timestamps
    origins = [ts for ts in test_df.index if ts.hour == 0]

    # Keep only origins whose full 24-hour forecast window stays inside test_df
    origins = [
    ...
    for i, origin in enumerate(origins, start=1):
        ...
        # Store the full next-day 24-hour forecast path
        forecast_window = f_idx[(f_idx >= test_start) & (f_idx <= test_end)]
        for ts in forecast_window:
            pred.loc[ts] = float(fc.loc[ts])
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--refit_every", type=int, default=1, help="Refit frequency in daily origins (1=daily, 7=weekly)")
